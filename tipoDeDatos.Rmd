...
El link que necesitas lo creas **publicando** tu hoja de datos eb formato CSV. Este es un link de ejemplo:
```{r}
# link a data:
...
Toda tabla de datos debe tener un campo único (clave o _key_). Ese campo debe ser tipo texto. El campo clave de esta tabla de *ID*, pero se ha leido como número entero. Podemos cambiarlo así:

```{r}
hsbCSV$ID=as.character(hsbCSV$ID)
```

Siguiendo las especificaciones de la metadata, vemos que R ha identificado bien todas las numéricas, desde LOCUS a CIV, descartando a CAR. R y SPSS identifican siempre los valores con decimales como numeros, salvo que algun valor está mal escrito en esa columna (sea un texto):
...
R denomina llama a las categóricas _factor_. El código para convertir estás variables en factor, debende si estas son Nominales u Ordinales. Ajustemos la única ordinal que hay, nivel socio economico. Sigue estos pasos:
...
Si haces lo correcto en SPSS, habrías obtenido esto:
...
Nótese que R, cuando ya sabe el tipo de variable, informar lo que corresponde, sin embargo, si a SPSS le pides descriptivos obtienes esto:
